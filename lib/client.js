...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    return {
      ...
      // exists, then runs apply with `ctx.remote.session` available. `slots` is
      // declared alongside it because the seat registration needs it.
      inject: ['slots', 'remote.session'],
      apply(ctx) {
