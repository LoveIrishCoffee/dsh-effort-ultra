    ...
    return {
      ...
      apply(ctx) {
        ...
        const registerWhenReady = (scope) => {
          ...
          const remoteSession = scope.get('remote.session')
          if (remoteSession === undefined || typeof remoteSession.modelCatalog !== 'function') return
        ...
        // this plugin back, and the seat is registered inside that scope so the
        // catalog call it makes cannot hit the injection guard.
        ctx.inject(['remote', 'remote.session'], (scope) => {
          registerWhenReady(scope)
        })
        // The remote face can arrive after this plugin loads, so re-check on the
        // service announcement too rather than depending on a particular boot
        // order. `registerWhenReady(ctx)` re-reads through ctx.get.
        ctx.on('internal/service', (serviceName) => {
          if (serviceName === 'remote.session') registerWhenReady(ctx)
        })
      },
    }
  },
