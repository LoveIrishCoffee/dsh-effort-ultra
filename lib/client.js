...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     */
    const createSessionDirectory = ({ remoteSession, sessionsOf, sessionId }) => {
      ...
      const select = async (selection) => {
        // Resolved per call, not captured: the sessions service is often absent
        // when the seat registers, and a captured `undefined` would make every
        // pick fail forever.
        const sessions = sessionsOf()
        if (sessions === undefined || typeof sessions.selectModel !== 'function') {
          const message = 'model selection is unavailable: the sessions service is not readable'
          store.set({ status: 'error', error: message })
          throw new Error(message)
        }
    ...
    return {
      ...
      inject: ['slots'],
      apply(ctx) {
        // Stage markers. "the bundle ran but apply never did" and "apply ran and
        // then stopped somewhere" need opposite investigations, and the previous
        // three attempts at this each guessed which one it was.
        try {
          console.log('[effort-ultra] apply entered; ctx=' + typeof ctx +
            ' slots=' + String(ctx.get('slots') !== undefined) +
            ' sessions=' + String(ctx.get('sessions') !== undefined) +
            ' remoteSession=' + String(ctx.get('remote.session') !== undefined))
        } catch (error) {
          console.error('[effort-ultra] apply entered but ctx probing threw:', error)
        }
        ...
        const bound = locale !== undefined && typeof locale.bind === 'function'
          ...
          : undefined
        console.log('[effort-ultra] stylesheet + dicts ready; waiting for remote.session')
        ...
        const registerWhenReady = (remoteSession) => {
          ...
          // NOTE: `sessions` is deliberately NOT a precondition here. It is read
          // once at apply time, and at that moment it is usually absent — the
          // previous version captured that `undefined` in a closure and required
          // it forever, so registration never happened even after every other
          // service had arrived. Whether a pick can be WRITTEN is a per-render
          // question, resolved lazily in `select` below.
          registered = true
        ...
        const tryRegister = () => {
          ...
          console.log('[effort-ultra] tryRegister: remote.session=' + String(remoteSession !== undefined) +
            ' canSelect=' + String(ctx.get('sessions') !== undefined))
          if (remoteSession === undefined) return false
          return registerWhenReady(remoteSession)
        }
        if (tryRegister()) return
        console.log('[effort-ultra] remote.session not readable yet; listening for internal/service')
