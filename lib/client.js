...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function EffortUltra(props) {
      ...
      // so between mount and the first populated snapshot there is nothing to
      // label the chip with. Ask for the load on mount instead of waiting for a
      // click, so that empty window closes on its own.
      useEffect(() => {
        if (available && typeof load === 'function') load()
      }, [available, load])
      ...
      const body = state.status === 'loading' && ladder === null
        ? h('div', { className: 'deu-note' }, t('loading'))
        : ladder === null
          ? h('div', { className: 'deu-note' }, t('noEfforts'))
