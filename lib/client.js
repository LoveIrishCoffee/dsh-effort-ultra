      ...
      apply(ctx) {
        ...
        const registerWhenReady = () => {
          ...
          ctx.slots.inject(SLOT_NAME, () => ctx.slots.register({
            ...
            locale: LOCALE_NS,
            inject: (sessionId) => {
              // `directoryFor` resolves the session's shared directory and throws
              // for a session it cannot resolve. The previous version swallowed
              // that error and returned a permanently-empty store, which looked
              // exactly like "the catalog never loaded" and hid the real cause
              // for several rounds. Report it, and remember it, so the seat can
              // retry instead of freezing on a dead store.
              let directory
              try {
              ...
              if (address !== undefined) {
                console.log('[effort-ultra][diag] addressed-subagent session — read-only seat: ' +
                  String(JSON.stringify(address)).slice(0, 140))
              }
              return {
                available,
        ...
        registerWhenReady()
        // The service can arrive after this plugin loads, so watch for it the
        // way the shipped plugins do instead of depending on injection order.
        ctx.on('internal/service', (serviceName) => {
          if (serviceName === 'modelDirectories') registerWhenReady()
        })
      },
    }
  },
