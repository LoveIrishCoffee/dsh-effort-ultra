...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const SEP = '\u0000'
    /**
     * Ordering among the seat's registrations. DSH resolves this slot to the
     * lowest-priority live entry, and the shipped model-selection entry sits at
     * 0, so anything below 0 takes the seat from it.
     *
     * It is deliberately *deferred behind* the shipped entry rather than ahead
     * of it. The shipped entry's component is not only a renderer: mounting it
     * is what drives the shared directory's first catalog load. Shadowing it
     * outright stalled that load, and the seat then read `loading` forever —
     * which is exactly what happened in the field.
     *
     * So this plugin takes the seat only when it has something real to show, and
     * otherwise gives it back. See `shouldTakeSeat`.
     */
    ...
    function EffortUltra(props) {
      ...
      // Yield the seat while the directory is still empty. This runs after every
      // hook (and after the retry effects above) so the hook count is identical
      // on both paths and the catalog load keeps being driven even while this
      ...
      apply(ctx) {
        ...
        const registerWhenReady = () => {
          ...
          ctx.slots.inject(SLOT_NAME, () => ctx.slots.register({
            ...
            locale: LOCALE_NS,
            inject: (sessionId) => {
              // `directoryFor` throws for an unknown session. Degrade to an
              // empty seat instead of taking the composer down with it.
              let directory
              try {
