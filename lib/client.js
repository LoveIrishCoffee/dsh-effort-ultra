...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const { useState, useMemo, useCallback, useEffect, useRef, useSyncExternalStore, createElement: h } = React
    ...
    function EffortUltra(props) {
      ...
      const state = useSyncExternalStore(
        useCallback((notify) => directory.subscribe(notify), [directory]),
      ...
      const ladder = useMemo(() => ladderOf(state), [state])
