...
// TEMP DIAGNOSTIC — proves whether the bundle executes at all, before any
// dependency resolution can park it. Remove once the seat is confirmed.
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Fallback store for a session whose directory could not be resolved.
     *
     * It reports `loading` rather than `ready` on purpose: the retry effect
     * only fires while a directory reads `loading`, so a `ready` fallback
     * silently disabled the one mechanism that could recover. It also keeps
     * the same key set as a real directory store, because a fallback with a
     * different shape is what disguised this failure as "the catalog never
     * loaded" for several rounds.
     */
    const EMPTY_STORE = {
      subscribe: () => () => {},
      getSnapshot: () => ({ current: null, routable: null, groups: [], failures: [], status: 'loading', error: null }),
    }
    ...
    function useDirectorySnapshot(props) {
      ...
      useEffect(() => {
        ...
        // TEMP DIAGNOSTIC — remove once the seat handover is confirmed in the
        // field. Reports the store's shape at subscribe time and on every
        // notification, which is what distinguishes "the catalog never loaded"
        // from "the component never heard about it".
        const at = cache.current.value
        console.log('[effort-ultra][diag] subscribe: status=' + String(at && at.status) +
          ' groups=' + String(at && at.groups ? at.groups.length : -1) +
          ' current=' + JSON.stringify(at && at.current) +
          ' keys=' + JSON.stringify(Object.keys(at ?? {})))
        bump((n) => n + 1)
        return directory.subscribe(() => {
          const next = directory.getSnapshot()
          console.log('[effort-ultra][diag] notify: status=' + String(next && next.status) +
            ' groups=' + String(next && next.groups ? next.groups.length : -1) +
            ' sameRef=' + String(next === cache.current.value))
    ...
    function EffortUltra(props) {
      ...
      const populated = state.status === 'ready' && state.groups.length > 0
      ...
      useEffect(() => {
        // Deliberately NOT gated on `available`. `available` means "this session
        // may CHANGE the selection"; gating the read on it meant an addressed
        // subagent session, where the directory's own `load()` refuses, could
        // never even try — so the seat had no data to show at all rather than
        // the read-only view it should have had. The attempt is harmless: a
        // refusal surfaces in the rejection log below.
        if (typeof load !== 'function') return undefined
        if (state.status !== 'loading') return undefined
        if (retries.current >= MAX_LOAD_RETRIES) {
          // Diagnosable dead end: the directory never left `loading` even after
          // every retry, which means the shared catalog's host call never
          // settles. Say so once instead of leaving a silent spinner.
          if (!reported.current) {
            reported.current = true
            console.warn('[effort-ultra] directory still loading after ' + String(MAX_LOAD_RETRIES) +
              ' retries — the model catalog host call never settled. snapshot=' +
      ...
      // result there.
      if (!tookSeat.current && !readOnly) {
        // TEMP DIAGNOSTIC — log only on transitions, so a re-render storm cannot
        // flood the console.
        const tag = String(state.status) + '/' + String(state.groups ? state.groups.length : -1) +
          '/' + String(available) + '/' + typeof load
        if (globalThis.__deuLastYield !== tag) {
          globalThis.__deuLastYield = tag
          console.warn('[effort-ultra][diag] yielding seat — snapshot: status=' + String(state.status) +
            ' groups=' + String(state.groups ? state.groups.length : -1) +
            ' error=' + String(state.error ?? null) +
            ' available=' + String(available) +
            ' loadType=' + typeof load +
            ' propKeys=' + JSON.stringify(Object.keys(props)))
        }
        return null
      }
    ...
    return {
      ...
      inject: ['slots'],
      apply(ctx) {
        console.log('[effort-ultra] apply invoked; modelDirectories=' +
          String(ctx.get('modelDirectories') !== undefined))
        ...
        const registerWhenReady = (scope) => {
          if (registered) return
          const models = scope.get('modelDirectories')
          if (models === undefined || typeof models.directoryFor !== 'function') return
          registered = true
          console.log('[effort-ultra] modelDirectories present — registering the seat')
          const scopedSlots = scope.get('slots')
          ...
          scopedSlots.inject(SLOT_NAME, () => scopedSlots.register({
            ...
            locale: LOCALE_NS,
            inject: (sessionId) => {
              // `directoryFor` builds a ModelDirectory on first use for a session,
              // and that construction reaches `ctx.remote.session`. Reaching
              // `ctx.remote` from a context that does not DECLARE the `remote`
              // injection throws "cannot get property 'remote' without inject" —
              // which is exactly what happened for every NEW session, while an
              // established session worked because its directory was already
              // cached and the constructor never ran. This scope is created with
              // that injection present (see the ctx.inject below).
              let directory
              try {
                directory = models.directoryFor(sessionId)
              } catch (error) {
                const reason = String(error && error.message ? error.message : error)
                console.warn('[effort-ultra] directoryFor failed for session ' +
              ...
              // directory, though: gating the load on this left the seat reading
              // an empty store forever. Read always; write only when no address
              // is retained.
              const available = address === undefined
              if (address !== undefined) {
                console.log('[effort-ultra] addressed-subagent session — read-only seat')
              }
              return {
                available,
        ...
        // `remote` and `remote.session` are declared HERE rather than on the
        // returned plugin. Declaring them at the top level would park the whole
        // plugin until both exist — and a parked plugin never runs apply(), which
        // is silent. ctx.inject waits for them in its own scope without holding
        // this plugin back, and the seat registered inside that scope is what
        // lets `directoryFor` reach the remote face it needs.
        ctx.inject(['remote', 'remote.session'], (scope) => {
          console.log('[effort-ultra] remote scope ready; registering the seat')
          registerWhenReady(scope)
        })
        // The directory service can arrive after this plugin loads, so watch for
        // it too rather than depending on a particular boot order.
        ctx.on('internal/service', (serviceName) => {
          if (serviceName === 'modelDirectories' || serviceName === 'remote.session') {
            registerWhenReady(ctx)
          }
        })
      },
    }
